package com.fufu.yygh.cmn.service.impl;

import com.alibaba.excel.EasyExcel;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.fufu.yygh.cmn.listener.DictListener;
...
public class DictServiceImpl extends ServiceImpl<DictMapper, Dict> implements DictService {
    ...
    public String getDictName(String s, String value) {
        //如果string为空，则直接根据value查询
        if(StringUtils.isNullOrEmpty(s)) {
            //直接根据value值查询
            QueryWrapper<Dict> wapper = new QueryWrapper<>();
            wapper.eq("value",value);
            Dict dict = baseMapper.selectOne(wapper);
            return dict.getName();
        } else {
            Dict codeDict = this.getDictByDictCode(s);
    ...
    private Dict getDictByDictCode(String dictCode) {
        QueryWrapper<Dict> wapper = new QueryWrapper<>();
        wapper.eq("dict_code",dictCode);
        Dict codeDict = baseMapper.selectOne(wapper);
        return codeDict;
    }
    //判断id下面是否有子节点
